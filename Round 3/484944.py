"""asung_mixed.py
Combines asung_best.py for HG + VF + VEV_4000 + VEV_4500 (binary MR / OPT_BIN),
with ratio-trader-style pair directional for VEV_5000-5500 (using HG/VF spread signal),
and a standing bid-at-0 for VEV_6000 / VEV_6500 (capture any free extrinsic value).
"""
    ...
    def run(self, state: TradingState):
        ...
        if OPT_BIN_ENABLE and cur_vf_dev is not None:
            ...
            for sym, K, frac in [('VEV_4500', 4500, OPT_4500_FRAC),
                ...
                opt_max = int(OPT_VOUCHER_CAP * frac)
                opt_orders: List[Order] = []
                if cur_vf_dev > OPT_BIN_THR:
                    target = -opt_max
                    need = cur_pos - target
                    if need > 0:
                        opt_orders.append(Order(sym, int(bw_o + 1), -need))
                elif cur_vf_dev < -OPT_BIN_THR:
                    target = opt_max
                    need = target - cur_pos
                    if need > 0:
        ...
        if hg_od is not None and vf_od is not None \
           ...
           and vf_od.buy_orders and vf_od.sell_orders:
            ...
            dev_v = hg_mid_v - PAIR_RATIO * vf_mid_v
            ...
            if not pvod_exited:
                if pvod_side is None:
                    if dev_v > PAIR_VOD_ENTRY:   pvod_side = 'lv'
                    elif dev_v < -PAIR_VOD_ENTRY: pvod_side = 'sv'
                tgt_pct = _pair_vod_target_pct(abs(dev_v))
                if pvod_side == 'lv' and dev_v > PAIR_VOD_ENTRY and tgt_pct > 0:
                    for sym in PAIR_VOD_SYMBOLS:
                        target_pos = int(tgt_pct * PAIR_VOD_LIMIT)
                        cur_pos = state.position.get(sym, 0)
                        if cur_pos < target_pos:
                            ...
                            if od_v and od_v.sell_orders:
                                ...
                                if ords: result[sym] = ords
                elif pvod_side == 'sv' and dev_v < -PAIR_VOD_ENTRY and tgt_pct > 0:
                    for sym in PAIR_VOD_SYMBOLS:
                        target_pos = -int(tgt_pct * PAIR_VOD_LIMIT)
                        cur_pos = state.position.get(sym, 0)
                        if cur_pos > target_pos:
